...
def run_collect():
    ...
    try:
        from collect_piyao import main as run_piyao
        run_piyao()
    except Exception as e:
        print(f"  ❌ 辟谣平台采集失败: {e}")
...
def generate_report():
    ...
    # 历史 bug：原 BASE_DIR/../references 解析为项目 scam-slayer-data/references 陈旧镜像。
    ...
    # 已辟谣库（官方辟谣平台，🟢 真相基准，与诈骗案例库分离）
    rumor_lib = os.path.expanduser("~/.workbuddy/skills/scam-slayer/references/rumor-library.md")
    rumor_count = 0
    if os.path.exists(rumor_lib):
        with open(rumor_lib, encoding="utf-8") as rf:
            rumor_count = sum(1 for line in rf if line.startswith("### "))

    report_lines += [
        f"",
        f"**数据总量**: {total_all} 条（不含已辟谣库）",
        f"",
        f"## 📗 已辟谣库（官方辟谣平台 · 与诈骗案例分离）",
        f"",
        f"- 累计辟谣条目：**{rumor_count} 条**（联合辟谣平台 + 科学辟谣，🟢 真相基准，不计入净归档）",
        f"- 文件：`references/rumor-library.md`",
        f"",
        f"## 🔍 风险分布",
        f"",
