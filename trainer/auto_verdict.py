...
# ====== 配置区 ======
# 权威数据目录（KB + 队列）统一指向 skill 的 references/。
# 历史 bug：原 BASE_DIR = dirname(__file__)/.. 解析为项目 scam-slayer-data/references
# （陈旧镜像，缺 wechat_queue，导致 auto_verdict 永远读到空队列、积压无法消解）。
# 协议明确规定队列与统计落在 ~/.workbuddy/skills/scam-slayer/references/collected_data/，
# 故此处直接锁定 skill 权威目录。
SKILL_REFERENCES = os.path.expanduser("~/.workbuddy/skills/scam-slayer/references")
COLLECTED_DIR = os.path.join(SKILL_REFERENCES, "collected_data")
CASE_LIBRARY = os.path.join(SKILL_REFERENCES, "case-library.md")
STATS_FILE = os.path.join(COLLECTED_DIR, "stats.json")

# 每次最多鉴别数量（避免单次消耗太多token）；本地规则引擎无外部 API，可放宽到 1000
MAX_PER_RUN = 1000
